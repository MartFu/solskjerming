...
export function createModuleRegistry(modules: ModuleResult<string>[]) {
    ...
    return {
        ...
        buildCreationIntent: (
            ...
            const parentId =
                context.parentId?.replace(/^drafts\./, "") ?? undefined;
        ...
        /** * A flat list of all unique internalRoles registered across all modules.
         * Useful for GROQ filters or schema validation.
         */
        allRoles: Array.from(byRole.keys()),

        /**
         * Checks if a given role belongs to a specific module key.
         * Useful for conditional fields in schemas.
         */
        isRoleInModule: (
            ...
            return entry?.module.key === moduleKey;
        },

        /**
         * Checks if a role is one of a specific set of roles.
         */
        hasRole: (role: string | undefined, roles: string[]): boolean => {
