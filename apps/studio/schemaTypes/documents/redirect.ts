...
export const redirect = defineType({
    ...
    fields: [
        ...
        defineField({
            ...
            validation: (rule) => [
                rule.required(),
                rule.custom<SlugValue>(
                    async (value, { document, getClient }) => {
                        ...
                        const destination = (document?.destination as SlugValue)
                            ?.current;
                        if (source === destination) {
                            return "Kilden og destinasjonen kan ikke ha samme URL";
                        }
                        const client = getClient({ apiVersion: API_VERSION });
                        const existingRedirect = await validateRedirectLoop(
                            client,
                            {
                                _id: document?._id ?? "",
                                slug: source,
        ...
        defineField({
            ...
            validation: (rule) => [
                rule.required(),
                rule.custom<SlugValue>(
                    async (value, { getClient, document }) => {
                        ...
                        const source = (document as unknown as Redirect)?.source
                            ?.current;
